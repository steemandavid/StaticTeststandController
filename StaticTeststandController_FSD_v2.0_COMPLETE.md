...
- WS2812 RGB LED
- Buzzer
- 19V LiPo battery with voltage monitoring
...
| 3 | Break wire 1  | Flame front sensing (V) |
| 4 | Break wire 2  | Flame front sensing (V) |
| 5 | Break wire 3  | Flame front sensing (V) |
| 6 | Break wire 4  | Flame front sensing (V) |
| 7 | Battery_voltage | Battery voltage sense (V) |
...
- `HALT`: Safe shutdown (requires power cycle to restart afterwards)
- `CHECK_IGNITER`: Pre-fire igniter check
- `CHECK_BREAKWIRES`: Pre-fire breakwire check
...
- Remove SSID and wifi password from the settings file from github commit. Or use a separate wifi settings file that is .gitignored

### 4.3 Data Logging

**CSV File Format:**

Filename: `YYYY-MM-DD_HH-MM-SS.csv` (e.g., `2026-02-03_14-30-15.csv`)
...
**Thresholds (configurable in settings.txt file):**
RSSI_WARNING_THRESHOLD -70  // dBm
RSSI_ERROR_THRESHOLD -85    // dBm
...
- Auto power-off circuit, based on idle time and critically low battery
...
Add these to settings_t:
RSSI_WARNING_THRESHOLD
RSSI_ERROR_THRESHOLD
...
**Watchdog monitors:**
- State machine heartbeat (0.5s timeout)
